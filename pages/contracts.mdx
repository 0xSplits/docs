...
We've broken this section into two categories.

- **Core contracts:** these are the building blocks upon which all payment flows
  are built. There are currently three core contracts: Split, Waterfall,
  Vesting.
- **Templates**: these are pre-built payment flows that stack multiple contracts
  together. Since these contracts were built by the Splits team they are
  supported natively within the app, however anyone is able to create their own
  templates since they're independent from the protocol.

## Flow of Funds

The below diagram shows the key components of the protocol and highlights the
different methods for moving funds between the components. As a
