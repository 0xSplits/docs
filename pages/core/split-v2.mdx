...
<Callout>
 - Splits [Explorer](app.splits.org) and [Lite](lite.splits.org) will continue to support distributing and withdrawing from existing v2.0 splits.
 - New splits created on either application going forward will be v2.1.
</Callout>

<Toggle title="2.1">
 - Pull splits now handle non-compliant ERC20 tokens (like USDT) that don't return a boolean on approval calls, fixing a limitation in v2.
