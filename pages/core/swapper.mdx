...
price and the swapper's discount. If the oracle is unable to determine a clearing price (i.e., it doesn't
support tokens it has received), the Swapper's owner can withdraw those tokens
at any time.
...
- Each Swapper is a payable smart contract with a beneficiary, output token (ETH
  or any ERC20), oracle, discount, and owner. The oracle is modular so be careful to **use
  a secure oracle** with sensible settings for your desired behavior. You can
  use whatever oracle contract you'd like. Splits currently offers one
  [oracle](/core/oracle), which uses Uniswap v3 TWAP to price assets.
- Tokens received by Swapper are held in the contract's balance until `flash` is
  ...
  determined by the oracle and discount) to the beneficiary in exchange for the balance in
