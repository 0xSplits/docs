...
- Each Swapper is a payable smart contract with a beneficiary, output
  token (ETH or any ERC20), oracle, and owner. The oracle is modular so
  be careful to **use a secure oracle** with sensible settings for your
...
- Each Swapper can be owned, which means the **owner has full control of the
  deployment**. The owner may change the beneficiary or oracle, pause the contract, and execute
