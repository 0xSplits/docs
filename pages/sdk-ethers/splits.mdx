...
Returns token balances for a given `userId`, separated out by each 0xSplits contract the user has received from.
Can optionally pass in a list of `contractIds` to filter results to just a set of contracts.
