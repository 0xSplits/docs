...
into a stablecoin and set aside for taxes. This template simply stacks a [Pass-Through Wallet](/core/pass-through) on top of a [Split](/core/split) on top of a number of [Swappers](/core/swapper).
...
- Related: [Split](/core/split), [Swapper](/core/swapper), [Pass-Through Wallet](/core/pass-through)
...
- Each Diversifier is a payable smart contract that uses a Split to
  ...
  specified output tokens. A Pass-Through Wallet sits on top and owns / controls all of the 
  underlying modules, allowing the Diversifier to function as a full wallet. That's all there is to it, so if that sounds
  complicated we recommend you review the Split, Swapper, and Pass-Through Wallet contracts.
- Each Diversifier can be owned, which means the owner has full control of the
  deployment. The owner may modify the Split, change the oracles and discounts, pause the
