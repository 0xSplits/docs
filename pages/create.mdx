...
- **Address:** Each contract has an address that can receive ETH and ERC20
  tokens. You can use this address anywhere you want funds split according to
  contract's rules, including as a recipient of other contracts.
- **Recipient:** Each contract has at least one recipient, to whom funds are
  ...
  capable of receiving ETH and ERC20 tokens.
...
- **Distribute:** Because all of the contracts follow a
...
There are three core building blocks upon which all payment flows within
0xSplits are created: Split, Waterfall, and Vesting. You can stack these blocks
...
flows we've also created [Templates](#templates), which stack stack frequently
used blocks together.
...
together. Since these are opinionated flows, there's a dedicated creation form
and detail view within the app for each template.
