...
Returns the same data as `getSplitEarnings`, but includes token data as well as
a formatted amount.
...
const response = await dataClient.getFormattedSplitEarnings(args)
```
...
const response = await dataClient.getFormattedUserEarnings(args)
```
...
Returns the same data as `getUserEarningsByContract`, but includes token data as
well as a formatted amount.
...
const response = await dataClient.getFormattedUserEarningsByContract(args)
```
