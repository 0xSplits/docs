...
### getSplitMetadataViaProvider

Returns all metadata for a given `splitAddress` by reading directly from the
blockchain via RPC provider. This is useful for chains that are not yet
indexed by the protocol subgraph. Works best with a provider that supports
large log range searches (e.g. Alchemy). Not supported for V1 splits on
Mainnet.
...
const response = await splitsClient.getSplitMetadataViaProvider(args)
```
...
```graphql
{
  split: Split # see the data client for the full Split type
}
```
