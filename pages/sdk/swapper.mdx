...
Swaps the given tokens for the Swapper's output token. Uses Splits' integration trader contract to trade with
UniswapV3.
...
Updates the scaled offer factor overrides of the swapper. Only callable by the Swapper owner.
NOTE: To remove an existing override you need to pass in a scaled offer factor percent of 100
which tells the contract to fallback to the default scaled offer factor.
...
Generating call data for the create swapper function would look like:
