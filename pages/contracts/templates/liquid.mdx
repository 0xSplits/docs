...
![Liquid Split diagram](./img/diagram_liquid.png)
...
mutable [Split](/contracts/core/split). You can integrate Liquid Splits into
your own NFT project by forking
...
- When a Liquid Split is created, an ERC-1155 contract is deployed and 1,000
  NFTs are minted to the initial recipients.
- Instead of sending funds to the Split contract itself, funds are instead sent
  to the NFT contract.
- Whenever a balance is distributed, the NFT contract will update the Split to
  match current NFT holders and immediately distribute the balance. This means
  that if there's a balance and you transfer your NFTs to Alice, Alice will be
  entitled to that share of the balance.
