...
automatically swapped into the specified output token according to the oracle's
clearing price.
...
- Each Swapper is a payable smart contract with a specified beneficiary, output
  ...
  behavior. 0xSplits currently has one
...
- Each Swapper can be owned, which means the **owner has full control of the
  ...
  making it a multisig if this contract is being used in any trustless setting.
