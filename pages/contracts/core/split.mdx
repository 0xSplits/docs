...
Split acts as an equity instrument by letting you define the pecent of _total
future value_ each recipient will earn. It's a payable smart contract that
distributes all ETH & ERC20 tokens it receives among recipients according to
pre-set ownership percentages.

- Create one at [split.new](https://app.0xsplits.xyz/new/split/)
- [Contracts](https://github.com/0xSplits/splits-contracts)
- [SDK](/dev/sdk/splits)
- [Example](https://app.0xsplits.xyz/accounts/0xaD30f7EEBD9Bd5150a256F47DA41d4403033CdF0/)
- [Audit](https://github.com/0xSplits/splits-contracts/blob/main/audit/0xSplits_A-1.pdf)

## How it works

- Each Split is a payable smart contract to which ETH and ERC20 tokens may be
  sent.
- Each Split has two or more recipients, each of which is assigned an ownership
  percent.
- Recipients, ownerships, and keeper fees are stored onchain as calldata and
  re-passed as args & validated via hashing when needed.
- Each Split gets its own address & proxy for maximum composability with other
  contracts onchain. For these proxies, we extended EIP-1167 Minimal Proxy
  Contract to avoid `DELEGATECALL` inside `receive()` to accept hard gas-capped
  `sends` & `transfers`.
